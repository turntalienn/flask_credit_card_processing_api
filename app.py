...
def card_details():
    credit_card_num = request.json["CreditCardNumber"]
    card_holder = request.json["CardHolder"]
    exp_date = request.json["ExpirationDate"]
    security_code = request.json["SecurityCode"]
    amount = request.json["Amount"]
    pay = Payment()
    response = pay.process_payment(credit_card_num, card_holder, exp_date, security_code, amount)
    return response
